	...
	def parse(self):
		...
		if root.tag.lower() == "testcases":
			for element in root:	
				...
				tip.set_input("<TestInstances>" + (element.text if element.text is not None else '') + ''.join(etree.tostring(child, pretty_print=False,method='c14n') for child in element if child.tag == "TestInstance") + "</TestInstances>")
				tis = tip.parse()
				if tis:
					try:
						for ti in tis:
