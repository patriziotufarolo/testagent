...
class Probe(object):
    ...
    def run(self,celeryobj,current_testcase,all_testcases,testcase):
        ...
        try:
            ...
            logger.error("Phase " + str(counter) + " got exception. Reverting probe's operations. Step back to "+ str(counter-1) +" Exception: " + repr(e))
        ...
        #prod = Producer(exchange=exchange)
        with celeryobj.app.producer_or_acquire(None) as prod:
            prod.publish(result_to_send,serializer='raw',routing_key='key-test',declare=[queue],exchange=exchange,retry=True)
        return finalResult